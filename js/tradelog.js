// âââ TRADE LOG â closed trade â data.json via GitHub Actions âââ
...
const validateClosedTrade = (t) => {
  ...
  for (const k of nums) if (!Number.isFinite(Number(t[k]))) return `${k} hibĂĄs vagy hiĂĄnyzik`;
  if (t.risk_usd > 30) return 'Risk USD nagyobb mint 30 â tiltott';
  if (t.rr_actual < 0) return 'RR hibĂĄs';
  return '';
};
...
const submitTradeLog = async () => {
  ...
  btn.disabled = true; btn.textContent = 'âł NaplĂłzĂĄs...';
  try {
    const beforeSha = await getDataJsonSha();
    await dispatchWorkflow(LOG_WORKFLOW_FILE, { trade: JSON.stringify(payload) });
    showRefreshStatus('Trade naplĂłzĂĄs elindult. VĂĄrakozĂĄs data.json commitra...', 'success');
    pollDataJsonChange({
      beforeSha,
      maxAttempts: 18,
      intervalMs: 10000,
      onDone: (data) => {
        ...
        render(data);
        showRefreshStatus('â Trade elmentve a data.json-be.', 'success');
        btn.textContent = 'Trade naplĂłzĂĄsa'; btn.disabled = false;
        ['ti-entry','ti-sl','ti-tp','ti-exit','ti-risk','ti-score','ti-note'].forEach(id => { const n = $('#'+id); if (n) n.value = ''; });
      },
      onTimeout: () => {
        showRefreshStatus('A naplĂłzĂĄs elhĂşzĂłdott. FrissĂ­ts rĂĄ kĂŠzzel 1 perc mĂşlva.', 'warning');
        btn.textContent = original; btn.disabled = false;
      }
    });
  } catch (e) {
    showRefreshStatus('NaplĂłzĂĄsi hiba: ' + e.message, 'error');
    btn.textContent = original; btn.disabled = false;
  }
};
