...
REQUIRED = ["datetime", "session", "direction", "setup_type", "entry", "sl", "exit",
            "risk_usd", "pl_usd", "rr_actual", "score", "allowed", "rule_compliance"]
...
def main():
    raw = os.environ.get("TRADE_JSON", "").strip()
    if not raw:
        fail("TRADE_JSON missing")
    try:
        trade = json.loads(raw)
    except Exception as e:
        fail(f"invalid TRADE_JSON: {e}")
    ...
    data["performance"] = {
        ...
        "rulebreak_count": len([t for t in log if t.get("rule_compliance") == "nem"]),
